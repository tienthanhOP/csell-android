...
public class Luban {
    ...
    public static String DEFAULT_DISK_CACHE_DIR = "cacheCsell";
    ...
    public static Luban compress(File file, File cacheDir) {
        if (!isCacheDirValid(cacheDir)) {
            throw new IllegalArgumentException("The " + cacheDir.toString() + " must be Directory");
        }
    ...
    private static File getPhotoCacheDir(Context context, String cacheName) {
        File cacheDir = new File(Environment.getExternalStorageDirectory().toString());
        if (!cacheDir.exists()) {
            cacheDir.mkdirs();
        }
